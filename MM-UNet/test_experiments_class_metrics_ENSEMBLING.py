...
def test_net(net1,net2,net3,
        ...
        net2.eval()
        net3.eval()
        mean_cross_entropy_test1=0
        dice_l1=0
        mean_cross_entropy_test2=0
        dice_l2=0
        mean_cross_entropy_test3=0
        dice_l3=0
        y_true=[]
        y_pred1=[]
        y_pred2=[]
        ...
        for i, item in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                mean_cross_entropy_test1+=criterion_test(probs1.cpu(),true_masks.cpu()).item()
                dice_l1+=dice_loss(probs1.cpu(), true_masks.cpu(), multiclass=True).item()
                mean_cross_entropy_test2+=criterion_test(probs2.cpu(),true_masks.cpu()).item()
                dice_l2+=dice_loss(probs2.cpu(), true_masks.cpu(), multiclass=True).item()
                mean_cross_entropy_test3+=criterion_test(probs3.cpu(),true_masks.cpu()).item()
                dice_l3+=dice_loss(probs3.cpu(), true_masks.cpu(), multiclass=True).item()
