...
dir_checkpoint_1 = Path('./checkpoints1_final2/')
dir_checkpoint_2 = Path('./checkpoints2_final2/')
dir_checkpoint_3 = Path('./checkpoints3_final2/')
dir_checkpoint_4 = Path('./checkpoints4_final2/')
dir_checkpoint_5 = Path('./checkpoints5_final2/')
...
def train_net(
    ...
    for k in range(1,6):
        ...
        if load_data==0:
            ...
            if save_data==1:
                torch.save(train_loader,"/mnt/sdc1/llaudato6/experiment"+str(k)+"/train_loader.pth")
                torch.save(test_loader,"/mnt/sdc1/llaudato6/experiment"+str(k)+"/val_loader.pth")
                print("DATALOADERS SAVED!")
                

        elif load_data==1:
            train_loader=torch.load("/mnt/sdc1/llaudato6/experiment"+str(k)+"/train_loader.pth")
            test_loader=torch.load("/mnt/sdc1/llaudato6/experiment"+str(k)+"/val_loader.pth")
        ...
        for epoch in range(1, epochs+1):
            ...
            with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
                for i,item in enumerate(train_loader):
                    ...
                    with torch.cuda.amp.autocast(enabled=amp):
                        pred_masks, lab_pred_embedding= net(image) 
                        ...
                        ce_pat=criterion_train(lab_pred_embedding.cpu(), label.cpu())
                        loss=dl_seg+ce_pat #+ce_seg
            ...
            for i, item in enumerate(test_loader):
                ...
                with torch.no_grad():
                    ...
                    loss_test=criterion_test(probs.cpu(),true_masks.cpu())+dice_loss(probs.cpu(), true_masks.cpu(), multiclass=True)
                    mean_loss+=loss_test.item()
                    for j in range(batch_size):
                        ...
                        dice_score += dice_coeff(compact,true_masks[j,label[j].item(),:,:]).item()
            ...
            if save_checkpoint:
                Path(checkpoints_directories[k-1]).mkdir(parents=True, exist_ok=True)
                torch.save(net.state_dict(), str(checkpoints_directories[k-1] / 'checkpoint_epoch{}_{}.pth'.format(epoch,epoch_loss/len(train_loader))))
